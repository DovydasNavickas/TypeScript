...
namespace FourSlash {
    ...
    export class TestState {
        ...
        public verifyCompletions(options: FourSlashInterface.VerifyCompletionsOptions) {
            ...
            const actualCompletionsEntries = ts.createMap<ts.CompletionEntry>();
            for (const entry of actualCompletions.entries) {
                if (actualCompletionsEntries.has(entry.name)) {
                    // TODO: GH#23587
                    if (entry.name !== "undefined" && entry.name !== "require") this.raiseError(`Duplicate completions for ${entry.name}`);
                }
                else {
                    actualCompletionsEntries.set(entry.name, entry);
                }
            }
            ...
            if (options.are) {
                ...
                if (options.includes) {
                    for (const include of toArray(options.includes)) {
                        ...
                        const found = actualCompletionsEntries.get(name);
